...
export class WorkerFontOutliner implements FontOutliner {
  ...
  outline(bytes: Uint8Array, text: string, options: FontOutlineOptions): Promise<FontOutlineResult> {
    const task = this.queue.then(() => this.run(bytes, text, options), () => this.run(bytes, text, options));
    this.queue = task.then(() => undefined, () => undefined);
    return task;
  }

  private run(bytes: Uint8Array, text: string, options: FontOutlineOptions): Promise<FontOutlineResult> {
    return new Promise((resolve, reject) => {
      const worker = new Worker(new URL("../../workers/font.worker.ts", import.meta.url), { type: "module" });
      ...
      worker.onmessage = (event: MessageEvent<ExportWorkerResponse>) => {
        ...
        else reject(new Error(event.data.type === "error" ? event.data.message : "Unexpected font worker response"));
      };
